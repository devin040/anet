...
class BaseHome extends Page {
	...
	myDraft(currentUser) {
		return {
			...
			query: { state: [Report.STATE.DRAFT, Report.STATE.REJECTED], authorId: currentUser.uuid },
			filters: [
	...
	myPending(currentUser) {
		return {
			...
			query: { authorId: currentUser.uuid, state: [Report.STATE.PENDING_APPROVAL]},
			filters: [
	...
	myOrgRecent(currentUser) {
		...
		return {
			...
			query: {
				orgId: currentUser.position.organization.uuid,
	...
	mySensitiveInfo() {
		const authorizationGroupId = (this.state.userAuthGroups.length ? this.state.userAuthGroups.map(f => f.uuid) : [-1])
		return {
